...
from models.tpcap import TPCap
...
def preprocess_image_whoops(img):
    # img = Image.open(img_path).convert('RGB')
    # img = np.array(img)
    if img.mode == 'RGBA':
        img = img.convert('RGB')
...
def validation_whoops(
    ...
    for example in examples['test']:
        ...
        print('GT: ', captions)
        image = example['image']
        image = preprocess_image_whoops(image).to(device)
...
def validation_nocaps(
    ...
    outdomain = []
    overall = []
    img_info = json.load(open('nocaps_val.json','r'))    ### your "nocaps_val.json" path
    model.eval()
